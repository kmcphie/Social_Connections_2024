...
# Can I run last years code
# Run visNetwork code
# Reproducible examples on Rstudio community
...
from <- responses_clean %>%
  select(sports) %>%
  group_by(sports) %>%
  mutate(from = case_when(sports == "No" ~ 1,
...
#   rename("to" = gap_year)
# 
# test_nodes <- data.frame(id = 1:309)
# test_edges <- data.frame(from = c(1:309), to = (1))
# 

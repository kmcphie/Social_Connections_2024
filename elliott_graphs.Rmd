...
loc_by_group_size <- responses_clean %>%
  ...
  facet_grid(~ location) +
  theme_bw() +
  labs(x = "Virtual to In-Person Conversion Count", y = "Percentage of First Year Class") +
...
graph_location_gs_means <- location_gs_means %>%
  ggplot(aes(x = location, y = mean_gs, fill = location)) +
  geom_bar(stat = "identity") +
  labs(x = "Location",
          y = "Mean Virtual Conversion Count",
          title = "Comparing Virtual Conversion Counts by Location",
          subtitle = "Respondents who appeared in others lists were more satisfied",
          caption = "Mean Total = 3.747") +
...
satg_tot <- responses_clean %>%
  ...
  mutate(gender = str_replace(gender, pattern = "Prefer not to say", 
                              replacement = "Other")) %>%
  group_by(satisfaction, group_size) %>%
  ggplot(aes(x = satisfaction, y = group_size, color = pre_o)) +
  geom_point() +
  geom_jitter() +
  facet_wrap(~ pre_o) +
  theme_bw() +
...
# help with satg_tot labels, maybe look at interactive/animating
...
# look at gender ratios between groups of 4 potentially

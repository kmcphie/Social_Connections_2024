...
library(readxl)
expectations <- read_excel("shiny_app/raw_data/expectations.xlsx")
...
dissonance1 <- expectations %>%
  ...
  labs(x = "Matches", y = "Count",
       ...
       subtitle = " Aligning respondent speculation on best ways to connect with people vs\n how respondents actually meet their top four friends")
  
dissonance1

top5_means <- expectations %>%
  select(best) %>%
  group_by(best) %>%
  count("Zoom lecture", wt = runs)
